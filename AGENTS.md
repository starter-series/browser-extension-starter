...
- `store.config.js` → Store-asset scenes (what `npm run capture:store` screenshots). Edit this config, not the generic harness in `scripts/store-assets/`.
...
- **Store assets**: `npm run capture:store` drives the *built* extension with Playwright (`scripts/store-assets/`) to produce CWS screenshots + promo tile + `demo.webm`, and extracts listing copy from `store-assets/STORE_LISTING.md` into `description.md`. The run doubles as a real-bundle smoke test (a screenshot only appears if that feature rendered). Loading an MV3 extension needs a headed Chromium — runs headed locally, under `xvfb-run` in CI. Generated images/video are gitignored; `store.config.js`, `store-assets/fixtures/`, `store-assets/templates/`, and `STORE_LISTING.md` are tracked.
